...
import { ArrowLeft, Upload, Star, Save } from 'lucide-react';
import { StudentDetail, Class, EducationYear } from '../types';
...
const EditStudent: React.FC = () => {
  ...
  const fetchStudent = async () => {
    try {
      const response = await fetch(`/api/students/detail/${studentId}`);
      ...
      setFormData({
        ...
        parents_together: data.parents_together || true,
        ...
        mother_work_address: data.mother_work_address || '',
        ...
        father_work_address: data.father_work_address || '',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      formDataToSend.append('class_id', student?.class_id?.toString() || '');
      formDataToSend.append('education_year_id', student?.education_year_id?.toString() || '');
      ...
      if (response.ok) {
        // Öğrenci listesini yenilemek için event tetikle
        window.dispatchEvent(new Event('storage'));
        navigate(`/student/${studentId}`);
      } else {
        const errorData = await response.json();
        throw new Error(errorData.error || 'Öğrenci güncellenirken hata oluştu');
      }
    } catch (error) {
    ...
    <div className="max-w-4xl mx-auto">
      <div className="bg-white rounded-lg shadow p-6">
        <div className="flex items-center space-x-4 mb-6">
          ...
          <div>
            <h1 className="text-2xl font-bold text-gray-900">Öğrenci Düzenle</h1>
          ...
          <div className="bg-gray-50 p-4 rounded-lg">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                {student.photo && !formData.photo && (
                  <p className="text-xs text-gray-500 mt-1">Mevcut fotoğraf korunacak</p>
                )}
              </div>
